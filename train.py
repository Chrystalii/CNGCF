...
class LOADData(torch.utils.data.Dataset):
    def __init__(self):
        ...
        self.pos_weight = torch.tensor([float(self.adj_train.shape[0] * self.adj_train.shape[0] - self.adj_train.sum()) / self.adj_train.sum()])
        self.norm = self.adj_train.shape[0] * self.adj_train.shape[0] / float((self.adj_train.shape[0] * self.adj_train.shape[0] - self.adj_train.sum()) * 2)
...
def gae_for(args):
    ...
    for epoch in range(args.epochs):
        ...
        #print('xxxx', features.shape, adj_norm.shape,adj_label.shape) #torch.Size([1, 2708, 1433]) torch.Size([2708, 2708])
        ...
        roc_curr, ap_curr, pre_curr, rec_curr, ndcg_curr = get_roc_score(hidden_emb, val_edges, val_edges_false, args.gdc, args.Ks)
        ...
        if((epoch+1) % args.monit == 0):
            ...
            hidden_emb = z_scaled.detach().cpu().numpy()
            roc_score, ap_score, pre, rec, ndcg = get_roc_score(hidden_emb, val_edges, val_edges_false, args.gdc, args.Ks)
